// dart format width=80

/// GENERATED CODE - DO NOT MODIFY BY HAND
/// *****************************************************
///  FlutterGen
/// *****************************************************

// coverage:ignore-file
// ignore_for_file: type=lint
// ignore_for_file: deprecated_member_use,directives_ordering,implicit_dynamic_list_literal,unnecessary_import

import 'package:flutter/widgets.dart';

class $ResourcesGen {
  const $ResourcesGen();

  /// Directory path: resources/skins
  $ResourcesSkinsGen get skins => const $ResourcesSkinsGen();
}

class $ResourcesSkinsGen {
  const $ResourcesSkinsGen();

  /// Directory path: resources/skins/debug
  $ResourcesSkinsDebugGen get debug => const $ResourcesSkinsDebugGen();

  /// Directory path: resources/skins/neon
  $ResourcesSkinsNeonGen get neon => const $ResourcesSkinsNeonGen();

  /// Directory path: resources/skins/standard
  $ResourcesSkinsStandardGen get standard => const $ResourcesSkinsStandardGen();
}

class $ResourcesSkinsDebugGen {
  const $ResourcesSkinsDebugGen();

  /// File path: resources/skins/debug/README.md
  String get readme => 'resources/skins/debug/README.md';

  /// File path: resources/skins/debug/manifest.json
  String get manifest => 'resources/skins/debug/manifest.json';

  /// List of all assets
  List<String> get values => [readme, manifest];
}

class $ResourcesSkinsNeonGen {
  const $ResourcesSkinsNeonGen();

  /// File path: resources/skins/neon/manifest.json
  String get manifest => 'resources/skins/neon/manifest.json';

  /// List of all assets
  List<String> get values => [manifest];
}

class $ResourcesSkinsStandardGen {
  const $ResourcesSkinsStandardGen();

  /// Directory path: resources/skins/standard/button_push
  $ResourcesSkinsStandardButtonPushGen get buttonPush =>
      const $ResourcesSkinsStandardButtonPushGen();

  /// Directory path: resources/skins/standard/button_toggle
  $ResourcesSkinsStandardButtonToggleGen get buttonToggle =>
      const $ResourcesSkinsStandardButtonToggleGen();

  /// Directory path: resources/skins/standard/display
  $ResourcesSkinsStandardDisplayGen get display =>
      const $ResourcesSkinsStandardDisplayGen();

  /// Directory path: resources/skins/standard/joystick
  $ResourcesSkinsStandardJoystickGen get joystick =>
      const $ResourcesSkinsStandardJoystickGen();

  /// Directory path: resources/skins/standard/knob
  $ResourcesSkinsStandardKnobGen get knob =>
      const $ResourcesSkinsStandardKnobGen();

  /// Directory path: resources/skins/standard/led
  $ResourcesSkinsStandardLedGen get led =>
      const $ResourcesSkinsStandardLedGen();

  /// File path: resources/skins/standard/manifest.json
  String get manifest => 'resources/skins/standard/manifest.json';

  /// Directory path: resources/skins/standard/multiple_button
  $ResourcesSkinsStandardMultipleButtonGen get multipleButton =>
      const $ResourcesSkinsStandardMultipleButtonGen();

  /// Directory path: resources/skins/standard/multiple_select
  $ResourcesSkinsStandardMultipleSelectGen get multipleSelect =>
      const $ResourcesSkinsStandardMultipleSelectGen();

  /// Directory path: resources/skins/standard/slider
  $ResourcesSkinsStandardSliderGen get slider =>
      const $ResourcesSkinsStandardSliderGen();

  /// Directory path: resources/skins/standard/sounds
  $ResourcesSkinsStandardSoundsGen get sounds =>
      const $ResourcesSkinsStandardSoundsGen();

  /// Directory path: resources/skins/standard/toggle_switch
  $ResourcesSkinsStandardToggleSwitchGen get toggleSwitch =>
      const $ResourcesSkinsStandardToggleSwitchGen();

  /// List of all assets
  List<String> get values => [manifest];
}

class $ResourcesSkinsStandardButtonPushGen {
  const $ResourcesSkinsStandardButtonPushGen();

  /// File path: resources/skins/standard/button_push/active.svg
  String get active => 'resources/skins/standard/button_push/active.svg';

  /// File path: resources/skins/standard/button_push/bg.svg
  String get bg => 'resources/skins/standard/button_push/bg.svg';

  /// List of all assets
  List<String> get values => [active, bg];
}

class $ResourcesSkinsStandardButtonToggleGen {
  const $ResourcesSkinsStandardButtonToggleGen();

  /// File path: resources/skins/standard/button_toggle/active.svg
  String get active => 'resources/skins/standard/button_toggle/active.svg';

  /// File path: resources/skins/standard/button_toggle/bg.svg
  String get bg => 'resources/skins/standard/button_toggle/bg.svg';

  /// List of all assets
  List<String> get values => [active, bg];
}

class $ResourcesSkinsStandardDisplayGen {
  const $ResourcesSkinsStandardDisplayGen();

  /// File path: resources/skins/standard/display/bg.svg
  String get bg => 'resources/skins/standard/display/bg.svg';

  /// List of all assets
  List<String> get values => [bg];
}

class $ResourcesSkinsStandardJoystickGen {
  const $ResourcesSkinsStandardJoystickGen();

  /// File path: resources/skins/standard/joystick/base.svg
  String get base => 'resources/skins/standard/joystick/base.svg';

  /// File path: resources/skins/standard/joystick/stick.svg
  String get stick => 'resources/skins/standard/joystick/stick.svg';

  /// File path: resources/skins/standard/joystick/widget.css
  String get widgetCss => 'resources/skins/standard/joystick/widget.css';

  /// File path: resources/skins/standard/joystick/widget.html
  String get widgetHtml => 'resources/skins/standard/joystick/widget.html';

  /// List of all assets
  List<String> get values => [base, stick, widgetCss, widgetHtml];
}

class $ResourcesSkinsStandardKnobGen {
  const $ResourcesSkinsStandardKnobGen();

  /// File path: resources/skins/standard/knob/base.svg
  String get base => 'resources/skins/standard/knob/base.svg';

  /// File path: resources/skins/standard/knob/indicator.svg
  String get indicator => 'resources/skins/standard/knob/indicator.svg';

  /// List of all assets
  List<String> get values => [base, indicator];
}

class $ResourcesSkinsStandardLedGen {
  const $ResourcesSkinsStandardLedGen();

  /// File path: resources/skins/standard/led/base.svg
  String get base => 'resources/skins/standard/led/base.svg';

  /// File path: resources/skins/standard/led/glow.png
  AssetGenImage get glow =>
      const AssetGenImage('resources/skins/standard/led/glow.png');

  /// List of all assets
  List<dynamic> get values => [base, glow];
}

class $ResourcesSkinsStandardMultipleButtonGen {
  const $ResourcesSkinsStandardMultipleButtonGen();

  /// File path: resources/skins/standard/multiple_button/bg.svg
  String get bg => 'resources/skins/standard/multiple_button/bg.svg';

  /// File path: resources/skins/standard/multiple_button/item.svg
  String get item => 'resources/skins/standard/multiple_button/item.svg';

  /// List of all assets
  List<String> get values => [bg, item];
}

class $ResourcesSkinsStandardMultipleSelectGen {
  const $ResourcesSkinsStandardMultipleSelectGen();

  /// File path: resources/skins/standard/multiple_select/bg.svg
  String get bg => 'resources/skins/standard/multiple_select/bg.svg';

  /// File path: resources/skins/standard/multiple_select/item.svg
  String get item => 'resources/skins/standard/multiple_select/item.svg';

  /// List of all assets
  List<String> get values => [bg, item];
}

class $ResourcesSkinsStandardSliderGen {
  const $ResourcesSkinsStandardSliderGen();

  /// File path: resources/skins/standard/slider/thumb.svg
  String get thumb => 'resources/skins/standard/slider/thumb.svg';

  /// File path: resources/skins/standard/slider/track.svg
  String get track => 'resources/skins/standard/slider/track.svg';

  /// List of all assets
  List<String> get values => [thumb, track];
}

class $ResourcesSkinsStandardSoundsGen {
  const $ResourcesSkinsStandardSoundsGen();

  /// File path: resources/skins/standard/sounds/.gitkeep
  String get aGitkeep => 'resources/skins/standard/sounds/.gitkeep';

  /// List of all assets
  List<String> get values => [aGitkeep];
}

class $ResourcesSkinsStandardToggleSwitchGen {
  const $ResourcesSkinsStandardToggleSwitchGen();

  /// File path: resources/skins/standard/toggle_switch/off.svg
  String get off => 'resources/skins/standard/toggle_switch/off.svg';

  /// File path: resources/skins/standard/toggle_switch/on.svg
  String get on => 'resources/skins/standard/toggle_switch/on.svg';

  /// File path: resources/skins/standard/toggle_switch/widget.css
  String get widgetCss => 'resources/skins/standard/toggle_switch/widget.css';

  /// File path: resources/skins/standard/toggle_switch/widget.html
  String get widgetHtml => 'resources/skins/standard/toggle_switch/widget.html';

  /// List of all assets
  List<String> get values => [off, on, widgetCss, widgetHtml];
}

class Assets {
  const Assets._();

  static const $ResourcesGen resources = $ResourcesGen();
}

class AssetGenImage {
  const AssetGenImage(
    this._assetName, {
    this.size,
    this.flavors = const {},
    this.animation,
  });

  final String _assetName;

  final Size? size;
  final Set<String> flavors;
  final AssetGenImageAnimation? animation;

  Image image({
    Key? key,
    AssetBundle? bundle,
    ImageFrameBuilder? frameBuilder,
    ImageErrorWidgetBuilder? errorBuilder,
    String? semanticLabel,
    bool excludeFromSemantics = false,
    double? scale,
    double? width,
    double? height,
    Color? color,
    Animation<double>? opacity,
    BlendMode? colorBlendMode,
    BoxFit? fit,
    AlignmentGeometry alignment = Alignment.center,
    ImageRepeat repeat = ImageRepeat.noRepeat,
    Rect? centerSlice,
    bool matchTextDirection = false,
    bool gaplessPlayback = true,
    bool isAntiAlias = false,
    String? package,
    FilterQuality filterQuality = FilterQuality.medium,
    int? cacheWidth,
    int? cacheHeight,
  }) {
    return Image.asset(
      _assetName,
      key: key,
      bundle: bundle,
      frameBuilder: frameBuilder,
      errorBuilder: errorBuilder,
      semanticLabel: semanticLabel,
      excludeFromSemantics: excludeFromSemantics,
      scale: scale,
      width: width,
      height: height,
      color: color,
      opacity: opacity,
      colorBlendMode: colorBlendMode,
      fit: fit,
      alignment: alignment,
      repeat: repeat,
      centerSlice: centerSlice,
      matchTextDirection: matchTextDirection,
      gaplessPlayback: gaplessPlayback,
      isAntiAlias: isAntiAlias,
      package: package,
      filterQuality: filterQuality,
      cacheWidth: cacheWidth,
      cacheHeight: cacheHeight,
    );
  }

  ImageProvider provider({
    AssetBundle? bundle,
    String? package,
  }) {
    return AssetImage(
      _assetName,
      bundle: bundle,
      package: package,
    );
  }

  String get path => _assetName;

  String get keyName => _assetName;
}

class AssetGenImageAnimation {
  const AssetGenImageAnimation({
    required this.isAnimation,
    required this.duration,
    required this.frames,
  });

  final bool isAnimation;
  final Duration duration;
  final int frames;
}
